        ...
        <div className="px-2 w-full">
            <div className=" flex flex-col items-center pt-4 w-full max-w-[1200px] mx-auto">
                ...
                <div className="bg-[#5e5e5e13] flex items-center justify-center gap-2 text-sm px-3 py-1 rounded-3xl font-medium mb-10 xl:text-base xl:mb-14">
                    <p> ⏱️{sla?.slaString?.toLowerCase() ?? "As fast as light"}</p>
                    <p className="pipe">|</p>
                ...
                <div className="w-full flex flex-col  justify-center gap-2">
                    {   dishes.map((dish , ind) => 
                        {  
                            return <Menu_Accordian 
                                info={dish.card.card} 
                                showFunction={()=>{ setShowIndex(ind)}} 
                                hideFuction={()=>{setShowIndex(-1)}} 
                                key={dish.card.card.title} 
                                showMenu={ind == showIndex ?  true : false} 
                            />
                        }
                    )}
                </div>
